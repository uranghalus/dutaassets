...
export async function getPendingNotifications() {
  ...
  else if (jabatan.includes("fa manager") || jabatan.includes("finance")) statusFilter = ["PENDING_FA"];
  else if (jabatan.includes("gm") || jabatan.includes("general manager")) statusFilter = ["PENDING_GM"];
  else if (jabatan.includes("admin") || jabatan.includes("warehouse")) statusFilter = ["PENDING_WAREHOUSE"];
  ...
  const notifications: AppNotification[] = pendingRequisitions.map((r) => ({
    ...
    message: `${r.requester.nama} requested items.`,
    date: r.createdAt,
