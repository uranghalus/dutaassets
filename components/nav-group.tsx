...
export function NavGroup({ title, items }: NavGroupProps) {
  ...
  // By convention, titles in sidebar-data use 'nav' namespace or bare string
  // useTranslations will return the key itself if not found, which handles untranslated items.
  const t = useTranslations("nav");
  // Handle nested group paths correctly like groups.general
...
function SidebarMenuLink({
  ...
  const t = useTranslations("nav");
  const translatedTitle = t(item.title as any);
        ...
        <CollapsibleContent>
          <SidebarMenuSub>
            {item.items.map((subItem) => {
              const translatedSubTitle = t(subItem.title as any);
              return (
    ...
    <SidebarMenuItem>
      <DropdownMenu>
        ...
        <DropdownMenuContent side="right" align="start" sideOffset={4}>
          ...
          <DropdownMenuSeparator />
          {item.items.map((sub) => {
            const translatedSubTitle = t(sub.title as any);
            return (
