...
async function main() {
  ...
  // 1. Clean orphans in Department (no valid organization_id)
  ...
  if (orphanDepartments.length > 0) {
    console.log(
      `Found ${orphanDepartments.length} orphan departments. Deleting...`,
    );
    await prisma.department.deleteMany({
      where: {
        id_department: { in: orphanDepartments.map((d) => d.id_department) },
      },
    });
  } else {
  ...
  if (orphanDivisiOrg.length > 0) {
    console.log(
      `Found ${orphanDivisiOrg.length} orphan divisions (invalid org). Deleting...`,
    );
    await prisma.divisi.deleteMany({
      where: { id_divisi: { in: orphanDivisiOrg.map((d) => d.id_divisi) } },
    });
  } else {
  ...
  if (orphanDivisiDept.length > 0) {
    console.log(
      `Found ${orphanDivisiDept.length} orphan divisions (invalid dept). Deleting...`,
    );
    await prisma.divisi.deleteMany({
      where: { id_divisi: { in: orphanDivisiDept.map((d) => d.id_divisi) } },
    });
  } else {
